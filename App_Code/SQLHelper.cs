...
namespace SQLHelper
{
	...
	public class SQLHelper
	{
        ...
        /// <summary>
        /// 执行add,delete,update之类不需要返回值的sql语句
        /// </summary>
        /// <param name="cmdText">sql语句</param>
        public void ExecSqlwithoutReturn(string cmdText)
        {
            if (myConnection == null)
            {
                myConnection = new SqlConnection(WebConfigurationManager.ConnectionStrings["Connection String"].ConnectionString);
        ...
        /// <summary>
        /// 执行select之类需要返回值的sql语句
        /// </summary>
        /// <param name="cmdText">sql语句</param>
        /// <returns>DataSet记录集</returns>
        public DataSet ExecSqlwithReturn(string cmdText)
        {
            if (myConnection == null)
            {
                myConnection = new SqlConnection(WebConfigurationManager.ConnectionStrings["Connection String"].ConnectionString);
            ...
            SqlDataAdapter ada = new SqlDataAdapter(cmdText, myConnection);
        ...
        /// 进行事务处理
        /// </summary>
        /// <param name="strSql">对数据库进行事务处理的查询字符串数组</param>
        public void RunTransaction(string[] strSql)
        {
            if (myConnection == null)
            {
                myConnection = new SqlConnection(WebConfigurationManager.ConnectionStrings["Connection String"].ConnectionString);				
            ...
            for (int x = 0; x < strSql.Length; x++)
            {
                myCommand.CommandText = strSql[x];
